...
public:
  ...
  struct KeyEvent {
    ...
    KeyEvent(Key k = Key::UNKNOWN, char c = '\0') : key(k), character(c) {}
  };

  static void print_colored(const std::string &text, Color color);
  static void print_styled(const std::string &text, Style style);
  static void print_formatted(const std::string &text, Color color,
                              Style style);
  static int get_key();
  static bool key_available();
  static std::pair<Key, char> get_input();
  static void draw_horizontal_line(int row, int start_col, int length,
                                   char ch = '-');
  static void draw_vertical_line(int start_row, int col, int length,
                                 char ch = '|');
  ...
  static std::pair<int, int> get_centered_position(int content_width,
                                                   int content_height);
  ...
  static std::pair<int, int> get_centering_margins(int content_width,
                                                   int content_height);
...
public:
  ...
  void clear_screen() { TerminalUtils::clear_screen(); }
  void flush_output() { std::cout.flush(); }

  std::optional<TerminalUtils::KeyEvent> get_key_input();
  bool key_available();
