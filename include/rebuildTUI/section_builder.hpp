...
public:
  explicit SectionBuilder(const std::string &name) : name_(name) {}
  ...
  SectionBuilder &
  add_generated_items(size_t count,
                      std::function<SelectableItem(size_t)> generator) {
    for (size_t i = 0; i < count; ++i)
      items_.push_back(generator(i));
    return *this;
  }

  template <typename T> SectionBuilder &user_data(const T &data) {
    user_data_ = data;
    return *this;
  }
  ...
  SectionBuilder &
  callbacks(std::function<void()> enter_cb, std::function<void()> exit_cb,
  ...
  SectionBuilder &set_item_callbacks(std::function<void(bool)> callback) {
    for (auto &item : items_)
      item.set_toggle_callback(callback);
    return *this;
  }

  SectionBuilder &apply_to_items(std::function<void(SelectableItem &)> func) {
    for (auto &item : items_)
      func(item);
    return *this;
  }
  ...
  SectionBuilder &
  filter_items(std::function<bool(const SelectableItem &)> predicate) {
    items_.erase(std::remove_if(items_.begin(), items_.end(),
                                [&predicate](const SelectableItem &item) {
  ...
  std::unique_ptr<Section> build_unique() {
    return std::make_unique<Section>(build());
  }

  std::shared_ptr<Section> build_shared() {
    return std::make_shared<Section>(build());
  }
...
public:
  ...
  MultiSectionBuilder &
  add_section(const std::string &name,
              std::function<void(SectionBuilder &)> configurator) {
