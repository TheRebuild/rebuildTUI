...
public:
  ...
  struct TextConfig {
    ...
    std::string help_text_sections =
        "Enter - select | q - quit | 1-9 - quick select";
   ...
   */
  using SectionSelectedCallback =
      std::function<void(size_t section_index, const Section &section)>;
  using ItemToggledCallback = std::function<void(
      size_t section_index, size_t item_index, bool selected)>;
  using PageChangedCallback =
      std::function<void(int new_page, int total_pages)>;
  using StateChangedCallback =
      std::function<void(NavigationState old_state, NavigationState new_state)>;
  using ExitCallback =
      std::function<void(const std::vector<Section> &sections)>;
  using CustomCommandCallback =
      std::function<bool(char key, NavigationState state)>;
...
public:
  NavigationTUI();
  explicit NavigationTUI(const Config &config);
  ...
  size_t get_current_section_index() const;
  Section *get_section(size_t index);
  const Section *get_section(size_t index) const;

  Section *get_section_by_name(const std::string &name);
  const Section *get_section_by_name(const std::string &name) const;
   ...
   */
  std::map<std::string, std::vector<std::string>> get_all_selections() const;

  /**
   * @brief Get selections for a specific section
   */
  std::vector<std::string> get_section_selections(size_t section_index) const;
   ...
   * Navigation state
   */
  NavigationState get_current_state() const;
   ...
   */
  size_t get_current_selection_index() const;
...
private:
   ...
   */
  void render_section_selection(int start_row, int left_padding,
                                int content_width);

  /**
   * @brief Render item selection screen
   */
  void render_item_selection(int start_row, int left_padding,
                             int content_width);

  /**
   * @brief Render header with title
   */
  void render_header(int term_width, int content_width,
   ...
   */
  int calculate_total_pages() const;
  std::pair<size_t, size_t> get_current_page_bounds() const;
  void clamp_selection();

  /**
   * @brief Rendering helpers
   */
  std::vector<std::string> get_section_display_items() const;
  std::vector<std::string> get_current_item_display_items() const;
  std::string format_item_with_theme(const SelectableItem &item,
                                     bool is_selected) const;
  std::string get_page_info_string() const;

  /**
   * @brief Layout calculation
   */
  std::pair<int, int> calculate_content_dimensions() const;
  std::tuple<int, int, int, int> get_content_area() const;
  std::pair<int, int> apply_centering_offset(int row, int col) const;
  int get_effective_content_width(int) const;
  int get_effective_content_height() const;
  ...
  std::string apply_theme_formatting(const std::string &text,
                                     const std::string &type) const;

  /*
   * @brief Center a string within a given width
   */
  std::string center_string(const std::string &text, int width) const;
};
...
public:
  ...
  NavigationBuilder &text_help(const std::string &section_help,
   ...
   */
  NavigationBuilder &
  on_section_selected(NavigationTUI::SectionSelectedCallback callback);
  NavigationBuilder &
  on_item_toggled(NavigationTUI::ItemToggledCallback callback);
  NavigationBuilder &
  on_page_changed(NavigationTUI::PageChangedCallback callback);
  NavigationBuilder &
  on_state_changed(NavigationTUI::StateChangedCallback callback);
  NavigationBuilder &on_exit(NavigationTUI::ExitCallback callback);
  NavigationBuilder &
  on_custom_command(NavigationTUI::CustomCommandCallback callback);
