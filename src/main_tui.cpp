...
std::vector<Section> generate_comprehensive_configuration() {
  ...
  auto privacy =
      SectionBuilder("Privacy & Security")
          ...
          .add_item("Disable Location Tracking",
                    "Stop apps from accessing location")
          .add_item("Clear Web Data", "Remove browsing history and cookies")
          .add_item("Disable Microphone Access",
                    "Prevent unauthorized microphone use")
          ...
          .on_enter([]() {
            std::cout << "🔒 Configuring privacy and security settings...\n";
          })
          .on_item_toggled([](size_t idx, bool selected) {
            std::vector<std::string> items = {"Block Telemetry",
                                              ...
                                              "VPN Integration"};
            if (idx < items.size()) {
              std::cout << "🔐 Privacy setting '" << items[idx] << "' "
              ...
              [](size_t i) -> SelectableItem {
                std::vector<std::pair<std::string, std::string>> optimizations =
                    {{"Disable Startup Programs",
                      "Reduce boot time by disabling unnecessary startup apps"},
                     {"Clear Temporary Files",
                      "Free up disk space by removing temp files"},
                     {"Optimize Memory Usage",
                      "Better RAM management and cleanup"},
                     {"Disable Visual Effects",
                      "Reduce GPU and CPU usage from animations"},
                     ...
                     {"Update Device Drivers",
                      "Install latest hardware drivers"},
                ...
                if (i < optimizations.size()) {
                  return SelectableItem(optimizations[i].first,
                                        optimizations[i].second,
                                        static_cast<int>(i));
                }
                return SelectableItem("Optimization " + std::to_string(i + 1),
                                      "Performance optimization option");
              })
          .select_items({"Clear Temporary Files", "Optimize Memory Usage",
                         "Update Device Drivers"})
          .sort_items()
          .on_enter([]() {
            std::cout << "⚡ Configuring performance optimizations...\n";
          })
          .build();

  auto customization =
      SectionBuilder("System Customization")
          ...
          .add_items(std::vector<std::pair<std::string, std::string>>{
              ...
              {"Taskbar Customization",
               "Modify taskbar appearance and behavior"},
          ...
          .on_enter(
              []() { std::cout << "🎨 Configuring system customization...\n"; })
          .build();

  auto dev_tools =
      SectionBuilder("Developer Tools")
          .description("Tools and settings for software development")
          .add_item("Enable Developer Mode",
                    "Access advanced development features")
          .add_item("Windows Subsystem for Linux",
                    "Run Linux environment on Windows")
          .add_item("Command Line Tools",
                    "Install terminal and shell utilities")
          .add_item("Package Managers", "Enable package management systems")
          .add_item("Git Version Control",
                    "Install Git for source code management")
          .add_item("Code Editor Integration", "Setup IDE and editor support")
          .add_item("Debugging Tools",
                    "Install application debugging utilities")
          .add_item("Performance Profilers",
                    "Tools for code performance analysis")
          .add_item("Container Support", "Docker and container runtime")
          .add_item("Virtual Machines", "Hypervisor and VM support")
          .on_enter([]() {
            std::cout << "👨‍💻 Configuring developer tools...\n";
          })
          .build();

  auto gaming =
      SectionBuilder("Gaming Optimization")
          ...
          .add_item("High Performance Power Plan",
                    "Maximum performance power settings")
          .add_item("Disable Windows Update", "Prevent updates during gaming")
          .add_item("Network Optimization",
                    "Reduce network latency for online games")
          .add_item("Audio Optimization", "Low-latency audio for gaming")
          .add_item("Fullscreen Optimizations",
                    "Disable fullscreen optimization")
          .select_items(
              {"Game Mode", "GPU Optimization", "High Performance Power Plan"})
          .on_enter(
              []() { std::cout << "🎮 Configuring gaming optimizations...\n"; })
          .build();

  std::vector<Section> temp_sections = {privacy, performance, customization,
                                        dev_tools, gaming};
  ...
  try {
    ...
    auto tui =
        NavigationBuilder()
            ...
            .text_help(
                "Up/Down: Navigate | Enter: Select | 1-9: Quick | Q: Quit",
            ...
            // .layout_fullscreen()
            // .layout_centering(
            //     true, true) // Center horizontally, top-aligned vertically
            // .layout_content_width(60, 80)  // Min 60, max 120 characters
            // width
            .layout_items_per_page(15) // Show 15 items per page
            // .layout_borders(true)      // Show borders around content
            ...
            .on_item_toggled(
                [](size_t section_idx, size_t item_idx, bool selected) {
                  std::cout << "🔄 Section " << section_idx << ", Item "
                            << item_idx << " is now "
                            << (selected ? "ENABLED" : "DISABLED") << std::endl;
                })
            .on_page_changed([](int new_page, int total_pages) {
              std::cout << "📄 Page changed to " << (new_page + 1) << " of "
                        << total_pages << std::endl;
            })
            .on_state_changed([](NavigationTUI::NavigationState old_state,
                                 NavigationTUI::NavigationState new_state) {
              std::vector<std::string> state_names = {"Section Selection",
                                                      "Item Selection"};
              std::cout << "🔄 Navigation state: "
                        << state_names[static_cast<int>(old_state)] << " → "
                        << state_names[static_cast<int>(new_state)]
                        << std::endl;
            })
            .on_custom_command([](char key,
                                  NavigationTUI::NavigationState /*state*/)
                                   -> bool {
              switch (key) {
              case '\n':
                return true;
              case 'h':
                ...
                std::cout
                    << "Custom shortcuts: s=save, r=reset, i=info, h=help\n\n";
                return true;

              case 's':
                std::cout << "\n💾 Configuration saved to profile!\n";
                std::cout
                    << "All your settings have been applied successfully.\n\n";
                return true;
              ...
              case 'i':
                ...
                std::cout
                    << "Supports any type of hierarchical configuration\n\n";
                return true;
            ...
            .on_exit([](const std::vector<Section> &sections) {
              ...
              if (total_selected == 0) {
                ...
                std::cout << "   • Total options selected: " << total_selected
                          << "\n";
                std::cout << "   • Sections configured: "
                          << total_sections_with_selections << " of "
                          << sections.size() << "\n";
              }
