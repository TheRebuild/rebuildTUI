...
#else
struct termios TerminalUtils::original_termios = {};
...
void TerminalUtils::clear_screen() {
  ...
  if (hConsole != INVALID_HANDLE_VALUE) {
    ...
    FillConsoleOutputCharacterA(hConsole, ' ', csbi.dwSize.X * csbi.dwSize.Y,
                                coord, &written);
...
void TerminalUtils::set_style(Style style) {
  ...
  if (hConsole != INVALID_HANDLE_VALUE) {
    ...
    case Style::BOLD:
      ...
      break;
    case Style::REVERSE:
      SetConsoleTextAttribute(hConsole, ((csbi.wAttributes & 0xF0) >> 4) |
                                            ((csbi.wAttributes & 0x0F) << 4));
      break;
    case Style::RESET:
...
void TerminalUtils::print_formatted(const std::string &text, Color color,
                                    Style style) {
  set_color(color);
  set_style(style);
  std::cout << text;
...
void TerminalUtils::draw_vertical_line(int start_row, int col, int length,
                                       char ch) {
  for (int i = 0; i < length; ++i) {
...
void TerminalUtils::print_centered(const std::string &text, int width,
                                   int row) {
...
std::pair<int, int> TerminalUtils::get_centered_position(int content_width,
                                                         int content_height) {
...
std::pair<int, int> TerminalUtils::get_centering_margins(int content_width,
                                                         int content_height) {
  auto [height, width] = get_terminal_size();
...
std::optional<tui::TerminalUtils::KeyEvent>
tui::TerminalManager::get_key_input() {
  ...
  TerminalUtils::Key converted_key = TerminalUtils::Key::NORMAL;
...
bool tui::TerminalManager::key_available() {
  return TerminalUtils::key_available();
}
